  ...
  <div class="container">
    <div style="height: 250px; width: 100%;">
      ...
      <el-card class="box-card card-container">
        ...
        <div class="form-row">
          <span class="form-label">模板名称：</span> 贝壳房源信息爬取
        </div>
        <div class="form-row">
          <span class="form-label">采集字段：</span> 楼盘名称，链接，分类，地址，面积，价格/每平，价格/整套
        </div>
        <div class="form-row">
          <span class="form-label">关键词示例：</span> gz (城市名每个字拼音首字母)
        </div>
...
export default {
  ...
  methods: {
    crawl() {
      ...
      axios.get(`http://localhost:8080/beike?key=${this.keyword}&page=${this.pagenum}`).then(res => {
        const taskData = {
          templateTitle: '贝壳房源信息爬取',
          templateTheme: '住房',
