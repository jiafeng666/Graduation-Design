...
module.exports = class BeiKeNewHouseExtractor {
    constructor() {
        this.userAgents = [
    ...
    async crawlList(keyword, pageNum) {
        ...
        for (let page=1; page<=pageNum; page++){
            let listingUrl = `https://${keyword}.fang.ke.com/loupan/pg${page}/`
            ...
            for(var j=0; j<houstList.length; j++) {
                ...
                var url = 'https://sz.fang.ke.com' + house.find('.resblock-name>a').attr('href');
                var cate = house.find('.resblock-name>span:last-child').text();
                var location = house.find('.resblock-location').text().trim();
                var area = house.find('.resblock-room').text().trim().replaceAll('\n', '').replaceAll('\t', '');
                // var contacts = house.find('.ke-agent-sj-name').text().trim();
